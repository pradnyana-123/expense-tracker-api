...
describe("ExpenseController", () => {
  ...
  describe("PATCH /api/expense/:userId/:categoryId/:expenseId", () => {
    beforeEach(async () => {
      await testService.deleteAll();
    });
    it("should assign category to expense successfully", async () => {
      // Create user
      const userResponse = await request(app.getHttpServer())
        .post("/api/users")
        .send({
      ...
      const response = await request(app.getHttpServer())
        .patch(`/api/expense/${userId}/${categoryId}/${expenseId}`)
        .expect(200);
      expect(response.body).toHaveProperty("message");
      expect(response.body.message).toBe("Expense assigned successfully");
    });
    it("should return 404 for non-existent expense", async () => {
      const userResponse = await request(app.getHttpServer())
        .post("/api/users")
      ...
      const categoryId = categoryResponse.body.id;
      await request(app.getHttpServer())
        .patch(`/api/expense/${userId}/${categoryId}/99999`)
        .expect(404);
    });
    it("should return 404 for non-existent category", async () => {
      ...
      const expenseId = expenseResponse.body.id;
      await request(app.getHttpServer())
        .patch(`/api/expense/${userId}/99999/${expenseId}`)
        .expect(404);
    });
    it("should return 403 when assigning category to other user expense", async () => {
      // Create two users
      const user1Response = await request(app.getHttpServer())
        .post("/api/users")
        .send({
      ...
      const categoryResponse = await request(app.getHttpServer())
        .post(`/api/categories/${user2Id}`)
        .send({
          name: "Food",
        });
      const categoryId = categoryResponse.body.id;
      // Try to assign user2's category to user1's expense
      await request(app.getHttpServer())
        .patch(`/api/expense/${user2Id}/${categoryId}/${expenseId}`)
        .expect(403);
    });
    it("should return 400 for invalid parameters", async () => {
      await request(app.getHttpServer())
        .patch("/api/expense/invalid/1/1")
        .expect(400);
      await request(app.getHttpServer())
        .patch("/api/expense/1/invalid/1")
        .expect(400);
      await request(app.getHttpServer())
        .patch("/api/expense/1/1/invalid")
        .expect(400);
    });
  });
});
